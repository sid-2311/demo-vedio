...
import {
  ShieldCheck, Lock, Mail, User, Calendar, Globe, AlertTriangle,
  ArrowRight, Key, CheckCircle2, Video, ShieldAlert, Sparkles, UserPlus, LogIn
} from 'lucide-react';
import { useAuth, HARDCODED_ACCOUNTS } from '../context/AuthContext';
import logoImg from '../assets/logo.png';

export const AuthPage = ({ defaultTab = 'signin', onSuccess, onSwitchTab }) => {
  ...
  return (
    <div className="min-h-[calc(100vh-68px)] flex items-center justify-center p-4 py-8 relative overflow-hidden bg-[#070913]">
      
      {/* Background ambient lighting */}
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[500px] h-[300px] bg-gradient-to-tr from-violet-600/20 via-purple-500/15 to-cyan-400/20 rounded-full blur-[100px] pointer-events-none" />

      <div className="w-full max-w-lg relative z-10 glass-panel rounded-3xl p-6 sm:p-8 border border-violet-500/30 shadow-2xl overflow-hidden">
        
        {/* Top Header Logo */}
        <div className="text-center mb-6">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-tr from-violet-600 via-purple-500 to-cyan-400 p-0.5 shadow-lg shadow-violet-500/30 mb-3">
            <div className="w-full h-full bg-slate-950 rounded-[14px] flex items-center justify-center">
              <ShieldCheck className="w-7 h-7 text-cyan-400" />
            </div>
          </div>
          <h1 className="text-2xl font-extrabold text-white tracking-tight">BETADRIX Account</h1>
          <p className="text-xs text-slate-400 mt-1">18+ Identity Verification & Matchmaking Portal</p>
        </div>

        {/* Tab Switcher */}
        <div className="flex items-center bg-slate-900/90 p-1 rounded-2xl border border-slate-800 mb-6">
          <button
            onClick={() => { setActiveTab('signin'); onSwitchTab && onSwitchTab('signin'); setAuthError(null); }}
          ...
          <button
            onClick={() => { setActiveTab('signup'); onSwitchTab && onSwitchTab('signup'); setAuthError(null); }}
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-all ${
              activeTab === 'signup'
                ? 'bg-gradient-to-r from-violet-600 to-purple-600 text-white shadow-md shadow-violet-500/20'
                : 'text-slate-400 hover:text-slate-200'
        ...
        {/* Auth Error Banner */}
        {authError && (
          <div className="mb-6 p-3.5 rounded-xl bg-rose-500/10 border border-rose-500/30 flex items-start gap-2.5 text-rose-300 text-xs animate-fadeIn">
        ...
        {activeTab === 'signin' && (
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {HARDCODED_ACCOUNTS.map((acc) => (
                  <button
                    ...
                    className={`px-2.5 py-2 rounded-xl text-[11px] font-semibold border transition-all flex flex-col items-center justify-center text-center ${
                      acc.isAdmin
            ...
            <div className="p-3.5 rounded-2xl bg-slate-900/80 border border-slate-800 text-[11px] space-y-2">
              <span className="font-semibold text-amber-400 block flex items-center gap-1">
                <Key className="w-3.5 h-3.5" /> Hardcoded Credentials Reference:
              </span>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 text-slate-400 font-mono text-[10px]">
        ...
        {activeTab === 'signup' && (
          <form onSubmit={handleSignUpSubmit} className="space-y-4">
            ...
            {/* Checkbox */}
            <label className="flex items-start gap-2.5 p-3 rounded-xl bg-slate-950/80 border border-slate-800 cursor-pointer">
              <input
                type="checkbox"
                required
