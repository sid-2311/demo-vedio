...
const TransportManager_1 = require("../managers/TransportManager");
function setupSocketHandler(io) {
    RoomManager_1.roomManager.setSocketIO(io);
    io.on('connection', (socket) => {
        ...
        socket.on('join-room', async ({ roomId, displayName }, callback) => {
            try {
                ...
                const room = await RoomManager_1.roomManager.getOrCreateRoom(roomId);
                const peer = PeerManager_1.peerManager.addPeer(socket.id, displayName || 'Stranger', roomId);
                socket.join(roomId);
                // Get list of existing peers and their producers in room
                const existingPeersInRoom = PeerManager_1.peerManager.getPeersInRoom(roomId).filter((p) => p.socketId !== socket.id);
        ...
        socket.on('get-room-producers', async ({ roomId }, callback) => {
            try {
                const peers = PeerManager_1.peerManager.getPeersInRoom(roomId).filter((p) => p.socketId !== socket.id);
                const producers = [];
        ...
        socket.on('create-transport', async ({ roomId, direction }, callback) => {
            try {
                const peer = PeerManager_1.peerManager.getPeer(socket.id);
                if (!peer)
                    throw new Error('Peer not found');
                const room = RoomManager_1.roomManager.getRoom(roomId);
                if (!room)
                    throw new Error('Room not found');
                const { transport, params } = await TransportManager_1.transportManager.createWebRtcTransport(room.router, socket.id, direction);
                if (direction === 'send') {
        ...
        socket.on('produce', async ({ transportId, kind, rtpParameters, appData }, callback) => {
            try {
                ...
                if (kind === 'audio') {
                    const room = RoomManager_1.roomManager.getRoom(peer.roomId);
                    if (room) {
        ...
        socket.on('set-consumer-preferred-layers', async ({ consumerId, spatialLayer, temporalLayer }, callback) => {
            try {
                ...
                console.log(`[Signaling] Consumer ${consumerId} preferred layers updated -> Spatial: ${spatialLayer}, Temporal: ${temporalLayer}`);
                if (callback)
                    callback({ success: true });
            }
            catch (error) {
        ...
        socket.on('restart-ice', async ({ transportId }, callback) => {
            try {
                ...
                const iceParameters = await TransportManager_1.transportManager.restartIce(transport);
        ...
        socket.on('disconnect', () => {
            ...
            const peer = PeerManager_1.peerManager.removePeer(socket.id);
            if (peer) {
                io.to(peer.roomId).emit('user-left', {
                    socketId: socket.id,
                    displayName: peer.displayName,
                });
                RoomManager_1.roomManager.closeRoomIfEmpty(peer.roomId);
            }
        });
    });
}
